				      ...
				      const double* trans)
{
	...
	*/
	...
	/// Begin of ghostlast l2g stuff
    } else
    {
    
	overlap.resize(cell_part.size());
	if (ghostLast < 4)
	    addOverlapLayer(grid, cell_part, overlap, my_rank, 0, true);
	else {
	    if (my_rank == 0) {std::cout << "No zero trans connections" << std::endl;}
	    addOverlapLayerNoTrans(grid, cell_part, overlap, my_rank, overlap_layers, true, trans);
	}
